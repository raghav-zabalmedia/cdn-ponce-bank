...
if(!$('#atmList').hasClass("show-list")) {
$('.mapboxgl-ctrl-bottom-right')[0].style.setProperty('bottom', '58px', 'important');
} else {
$('.mapboxgl-ctrl-bottom-right')[0].style.setProperty('bottom', ($('.atm_content').height() - 39) + 'px', 'important');   
}
});
...
await resObject.map(async (obj) => {
  ...
  cloneMapObj.properties.title =
    obj.atmLocation && obj.atmLocation.name ? capitalizeWords(obj.atmLocation.name): "";
