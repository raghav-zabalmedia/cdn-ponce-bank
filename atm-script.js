    ...
    document.querySelectorAll(".atm_item").forEach((item) => {
      item.addEventListener("click", async (e) => {
        ...
        if(window.innerWidth < 479) {
          ...
          if(atmDetailLink || atmDirection || atmDetail || e.target.classList.contains('atm-detail-link') || e.target.classList.contains('atm-direction') || e.target.classList.contains('atm_detail')) {
            return true;
          }
        }
        ...
        console.log('mobile_close-trigger')
        setTimeout(() => {
          if(window.innerWidth < 479) {
